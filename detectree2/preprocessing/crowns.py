...
def get_inner_rectangle_corner_coordinates_from_polygon(step_size,polygon):
    ...
    while(rectangle_size != 0):
        ...
        if all(point_inside_shape(geometry.Point(point),polygon) for point in rectangle_coordinates):
            return result_points
        else: 
            # rectangle is outside of polygon
            rectangle_coordinates = rotated_square(centroid.x,centroid.y,rectangle_size,degrees=0)
...
def clip_crown_from_raster(img_path:str,polygon,out_file_suffix:str):
    ...
    out_dir = Path(img_path).parent.parent / 'clipped_raster_files'
...
def clip_multiple_crowns_from_raster(img_path,crowns:GeoDataFrame):
...
def get_gdf_with_inner_rectangle_polygons(crowns:GeoDataFrame,step_size=1) -> GeoDataFrame:
    
    crowns['rec_poly'] = [get_inner_rectangle_corner_coordinates_from_polygon(step_size,polygon) for polygon in crowns['geometry']]
